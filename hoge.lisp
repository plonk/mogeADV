...
(defun text-set (p)
  (let* ((s1 (cadr (assoc (player-scene p) *text*)))
    ...
    (setf (player-text p)      (cadr (assoc 'text s1))
	  ...
	  (player-ending p)    (cadr (assoc 'end s1))
...
(defun game-init (p)
  ...
  (setf (player-text p)      nil
	(player-ask p)       nil
	(player-ask_skill p) nil
	(player-tp p)        0
	(player-mogep p)     0
	(player-ending p)    nil
	(player-scene p)     0
...
(defun game-start (p)
  (let* ((f1 (make-instance 'frame))
    ...
    (skill-point p f1) ;;技能ポイント設定 （仮）
    (text-set p)
    (show-text-set-next p l1)
    (pack lf1);;:fill :both :expand t)
